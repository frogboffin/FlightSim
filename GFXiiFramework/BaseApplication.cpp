...
int BaseApplication::Run()
{
	...
	while ( !m_terminate )
	{
		...
		if (lastRenderTime <= currentTime - (1000.0 / fps))
		{
			m_appwnd->Render();
...
LRESULT CALLBACK BaseApplication::WndProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	switch ( msg )
	{
		...
		case WM_KEYDOWN:
			s_oglapp->GetApplicationWindow()->GetKeyDown(GET_KEYSTATE_WPARAM(wparam));
			break;

		case WM_KEYUP:
			s_oglapp->GetApplicationWindow()->GetKeyUp(GET_KEYSTATE_WPARAM(wparam));
			break;

		case WM_MOUSEWHEEL:
			s_oglapp->GetApplicationWindow()->MouseWheel(GET_WHEEL_DELTA_WPARAM(wparam));
			break;
